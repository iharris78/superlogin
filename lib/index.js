...
module.exports = function (configData, passport, userDB, couchAuthDB) {
  ...
  var user = new User(config, userDB, couchAuthDB, mailer, emitter, onCreateActions, onLinkActions, adapter);
  ...
  localConfig(config, passport, user, userDB);
  // Load the routes
  loadRoutes(config, router, passport, user, couchAuthDB, mailer, emitter, onCreateActions, onLinkActions, adapter);
